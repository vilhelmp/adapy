   ...
   Outputs:
     ...
     .perror
        ...
        *If* you can assume that the true reduced chi-squared value is unity --
        ...
        if xall is None:
            #print self.parinfo(parinfo,'value')
        ...
        if (limited is not None) and (limits is not None):
            ...
            if numpy.any((limited[:,0] & (xall < limits[:,0])) |
                ...
                wrong_limit = limits[((limited[:,0] & (xall < limits[:,0])) | (limited[:,1] & (xall > limits[:,1])))]
                self.errmsg = 'ERROR: parameters are not within PARINFO limits : '+str(wrong_limit)
                #print xall
                #print limited[:,0]
                #print limited[:,1]
                return
