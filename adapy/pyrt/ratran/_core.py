...
class Output(object):
    ...
    def _check_molfile(self, molfile):
        if molfile: # if moldata file input
    ...
    def plot_his_tex(self, trans = [12, 10], runjump = 10):    # specify which transition
        """
        ...
        [_pl.plot(radii , j, color=str(c), lw=1, marker='o', ms=4, mew=0) for (j,c) in zip(tex, linspace(0.7,0,len(tex)))]
        
        
        ### get the final tex curve
        tex_final = temp_pop(self.Pop.lp[trans[1]-1], self.Pop.lp[trans[0]-1], gweights[1], gweights[0], nu)
        _pl.plot(radii , tex_final, label=r' - '.join([self.Moldata.get_lvl(i, tex = 1) for i in trans]), color=_gc(1).next(), lw=2, marker='o', ms=5, mew=0)
        _pl.legend()
        _pl.grid()
