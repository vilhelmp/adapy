        ...
        for par, stdval, unit in zip(params[0::3],params[1::3], params[2::3]):
            if par in kwargs:
                if par in ['silent', 'spacing', 'rho_type', 'opacfile', 'freqfile']:
                    ...
                    # change to this way instead, much better!!!
...
#~ 
        ...
        #~ gas2dust : Gas-to-Dust ratio
#~ 
        #~ ----------------------------------------------------------------
        #~ Refinement grid is only for Transphere. Good if interested in
        #~ small scales, close to the source.
        #~ ----------------------------------------------------------------
        #~ TODO:
...
#~ 
        ...
        #~ for par, stdval, unit in zip(params[0::3],params[1::3], params[2::3]):
            #~ if par in kwargs:
                ...
                #~ else:
                    #~ print '   {0:9} : {1} {2}'.format(par, kwargs[par], unit)
                    #~ exec('self.{0} = {1}'.format(par, float(kwargs[par])))
            #~ else:
                #~ if stdval != None:
                    #~ print '   {0:9} : {1:7} {2:4} \t(default)'.format(par, stdval, unit)
                    #~ if par in ['spacing', 'rho_type']:
                        #~ exec('self.{0} = {1}'.format(par, '\"'+stdval+'\"'))
                    #~ else:
                        #~ exec('self.{0} = {1}'.format(par, float(stdval)))
                #~ else:
                    #~ raise Exception('Wrong parameter input/handling'
                                    #~ 'Check input and/or code...')
        #~ # Convert distances to CGS units
...
#~ 
        ...
        #~ with open('envstruct.inp','w') as f:
            ...
            #~ for ir in range(0,len(self.Model.radii)):
                #~ f.write("%13.6E %13.6E %13.6E" % (self.Model.radii[ir], self.Model.rho_dust[ir], 0.e0)+'\n') # ,format='(3(E13.6,1X))'
#~ 
    #~ def runTransphere(self):
        #~ import os
        #~ from time import time
